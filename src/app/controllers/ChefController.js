...
module.exports = {
    async list(req, res) {
        ...
        return res.render("admin-access/chef/list", { chefs, session: req.session, sessionIsAdmin });
    },

    create(req, res) {
        return res.render("admin-access/chef/create", { session: req.session });
    },
    ...
    async post(req, res) {
        ...
        for (key of keys) {
            if (req.body[key] == "" && key != "information") {
                return res.render("admin-access/chef/create", { 
                    session: req.session,
        ...
        return res.render("admin-access/chef/show", { 
            chef, recipes, 
            files, 
            session: req.session, 
            sessionIsAdmin,
    ...
    async show(req, res) {
        ...
        return res.render("admin-access/chef/show", { chef, recipes, files, session: req.session, sessionIsAdmin });
    },
    ...
    async edit(req, res) {
        ...
        return res.render("admin-access/chef/edit", { chef, files, session: req.session });
    },
        ...
        try {
            ...
            for (key of keys) {
                if (req.body[key] == "" && key != "removed_files") {
                    return res.render("admin-access/chef/edit", { 
                        session: req.session,
                        error: "Preencha todos os campos",
                        chef: req.body,
                    });
    ...
    async delete(req, res) {
        ...
        if (results.rows[0].total_recipes == 0) {
            ...
            return res.render("admin-access/chef/list", { 
                chefs, 
                session: req.session, 
                sessionIsAdmin,
